...
showDialog = (function () {
    var j = [],
        ...
        isIE6 = (/MSIE 6.0/ig.test(NA)), isIE9 = (/MSIE 9.0/ig.test(NA)),
        isCHROME = (NU.indexOf('chrome') == -1),
        objWin = window, objDoc = document,
        objBody = objDoc.body,
    ...
    function setStyle(e, a) {
        var i;
        for (i in a) { e.style[i] = a[i] }
    }
    function getStyle(e, a) {
        var b = (typeof objDoc.defaultView == 'function') ? objDoc.defaultView() : objDoc.defaultView, s;
        if (b && b.getComputedStyle) { s = b.getComputedStyle(e, null); return s && s.getPropertyValue(a) }
    ...
    function getMaxH() {
        var a = getPageHeight(), wh = getWinHeight(); return Math.max(a, wh)
    }
    function getMaxW() {
        var a = getPageWidth(), ww = getWinWidth(); return Math.max(a, ww)
    }
    ...
    return {
        show: function (p) {
            ...
            this.sTime = a.sTime || null; this.delayPop = a.delayPop || false;
            ...
            if (b == 1) {
                ...
                this.src = arguments[0]; this.iWidth = arguments[1]; this.iHeight = arguments[2]
            }
            if (this.id == null && this.src == null && this.sMsg == null) { return }
            if (this.src != null) { this.id = sIframeId }
            if (this.sMsg != null) { this.id = sPopupMsgId }
            if (this.id != null) { j.push(this.id) }
            this.popupInit()
        },
        popupInit: function () {
            var a = this.onPopupCallback, b, c, d, f, g, k, h, _length, _src;
            if (a != null && typeof (a) === 'function') { a() }
            b = this.id, _src = this.src;
            if (!hasOverlay) {
                c = gEl(sOverLayId);
                if (c) { setStyle(c, { backgroundColor: this.bgcolor, display: 'block' }) }
                else { this.overlay() }
            }
            if (_src != null) {
                if (!gEl(sIframeId)) { d = objDoc.createElement('div'); objOverLay.parentNode.insertBefore(d, objOverLay) }
                else { d = gEl(sIframeId).parentNode }
                d.style.display = 'none';
                d.innerHTML = '<iframe onload="showDialog.ifrAutoHeight(this)" ' +
                    'style="display:block" scrolling="no" id="' + sIframeId + '" src="' + _src + '" width = "'
                    + this.iWidth + '" height = "' + this.iHeight +
                    '" frameborder = "0" allowtransparency="true"></iframe>';
                if (!this.delayPop) { d.style.display = 'block' }
            }
            if (this.sMsg != null) {
                if (!gEl(sPopupMsgId)) {
...
showDialog.showLoading = function () {
    showDialog.show({
        sMsg: "<img src='load.gif'/><span style='margin-left:5px;font-size:12px;display:inline-table;vertical-align:middle;color:222222;'>请稍后，数据正在提交中，请勿刷新页面...</span>",
